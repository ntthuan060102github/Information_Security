...
BEGIN
    ...
    SELECT USERNAME
    FROM ALL_USERS;
--    WHERE CREATED >= TO_DATE('22-03-21', 'YY-MM-DD');
...
BEGIN
    ...
    FROM SYS.DBA_ROLES
    WHERE ROLE LIKE 'ROLE%';
	--Các ROLE m?nh thêm s? có d?ng ROLE_<rolename> nha
	--ð? l?c b?t m?y cái role DBA
END;
/
...
-- Hi?n th? danh sách các quy?n c?a user (Do user ATBM_QLNV t?o).
CREATE OR REPLACE PROCEDURE SP_VIEW_USER_PRIVILEGE(
    I_USERNAME  VARCHAR2,
    I_USERTYPE  VARCHAR2,
    O_CURSOR    OUT SYS_REFCURSOR
) IS
BEGIN
    OPEN O_CURSOR FOR
    SELECT TABLE_NAME "OBJECT_NAME", '' "COLUMN", PRIVILEGE, GRANTABLE, TYPE
    FROM DBA_TAB_PRIVS
    WHERE OWNER = 'ATBM_QLNV' AND PRIVILEGE NOT IN ('INSERT', 'UPDATE') AND GRANTEE = I_USERNAME
    
    UNION 
    SELECT TABLE_NAME "OBJECT_NAME", COLUMN_NAME "COLUMN", PRIVILEGE, GRANTABLE, 'TABLE' "TYPE"
    FROM DBA_COL_PRIVS
    WHERE OWNER = 'ATBM_QLNV' AND GRANTEE = I_USERNAME
    
    UNION
    SELECT GRANTED_ROLE "OBJECT_NAME", '' "COLUMN", '' "PRIVILEGE", '' "GRANTABLE", 'ROLE' "TYPE"
    FROM DBA_ROLE_PRIVS
    WHERE GRANTED_ROLE LIKE 'ROLE%' AND GRANTEE = I_USERNAME
