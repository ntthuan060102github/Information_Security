...
--Proc thu hoi quyen he thong tu user/role
CREATE OR REPLACE PROCEDURE SP_REVOKE_SYS_PRIVS
(SUBJECT IN VARCHAR2,
PRIVS IN VARCHAR2)
AS
    Q VARCHAR2(1000);
BEGIN
    Q := 'REVOKE '||PRIVS||' FROM '||SUBJECT;
    EXECUTE IMMEDIATE 'ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE';
    EXECUTE IMMEDIATE Q;
END;
/

--Proc thu hoi quyen doi tuong tu user/role
CREATE OR REPLACE PROCEDURE SP_REVOKE_TAB_PRIVS
(SUBJECT IN VARCHAR2,
OBJ IN VARCHAR2,
PRIVS IN VARCHAR2)
AS
    Q VARCHAR2(1000);
BEGIN
    Q := 'REVOKE '||PRIVS||' ON '||OBJ||' FROM '||SUBJECT;
    EXECUTE IMMEDIATE 'ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE';
    EXECUTE IMMEDIATE Q;
END;
/
...
BEGIN
    EXECUTE IMMEDIATE 'ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE';
    EXECUTE IMMEDIATE 'ALTER USER ' || USERNAME || ' ACCOUNT ' || ACTION_TYPE;
END;
/

CREATE OR REPLACE PROCEDURE SP_MODIFY_PASSWORD(
    I_USERNAME IN VARCHAR2,
    I_PASSWORD IN VARCHAR2
)
IS
BEGIN
    EXECUTE IMMEDIATE 'ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE';
    EXECUTE IMMEDIATE 'ALTER USER ' || I_USERNAME || ' IDENTIFIED BY ' || I_PASSWORD;
END;
